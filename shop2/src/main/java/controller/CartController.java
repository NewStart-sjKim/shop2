...
public class CartController {
	...
	public ModelAndView delete(int index,HttpSession session) {
		...
		mav.addObject("message",robj.getItem().getName()+"가(이) 삭제되어습니다.");
		mav.addObject("cart",cart);
		return mav;
	}
	@RequestMapping("cartView")
	public ModelAndView view(Integer id, Integer quantity,HttpSession session) {
		ModelAndView mav = new ModelAndView("cart/cart");
		Cart cart = (Cart)session.getAttribute("CART");
		mav.addObject("cart",cart);
		return mav;
	...
	public String checkout(HttpSession session) {
		...
		//cart.geteItemSetList().size() : CART 속성은 존재. CART 내부에 상품정보가 없는 경우
	 ...
	 * 2. 장바구니상품 saleitem 테이블에 저장. 주문정보(sale) 테이블 저장 =>sevice.check
	 * 3. 장바구니 상품 제거.
